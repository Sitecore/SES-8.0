// -------------------------------------------------------------------------------------------
// <copyright file="NameProductUrlProcessor.cs" company="Sitecore Corporation">
...
namespace Sitecore.Ecommerce.Catalogs
{
  ...
  /// </summary>
  public class NameProductUrlProcessor : ProductUrlProcessor
  {
    /// <summary>
    /// Initializes a new instance of the <see cref="NameProductUrlProcessor"/> class.
    /// </summary>
    /// <param name="searchProvider">The search provider.</param>
    public NameProductUrlProcessor(ISearchProvider searchProvider) : base(searchProvider)
    {
    }
    ...
    public override UrlString GetProductUrl(Item catalogItem, Item productItem)
    {
      ...
      url.Path = string.Concat(IO.FileUtil.MakePath(url.Path, HttpUtility.UrlPathEncode(productItem.Name), '/'), ".aspx");
